...
const LegalPage = () => {
    ...
    useEffect(() => {
        ...
        getPublishedLegalDocuments()
            .then(docs => {
                console.log('[LegalPage] slug:', currentSlug, 'docs:', docs.length, docs.map(d => d.slug));
        ...
        return (
            <div css={tw`min-h-screen flex items-center justify-center`}
                ...
                <div css={tw`text-center p-8 max-w-md`}>
                    <h1 css={tw`text-4xl font-bold mb-4`}
    ...
    return (
        <div css={tw`min-h-screen`}
            ...
            <div css={tw`w-full px-8 lg:px-16 py-12 sm:py-16`}>
                ...
                <div css={tw`rounded-xl p-8 sm:p-12 xl:p-16 shadow-lg w-full`}
                     style={{ backgroundColor: 'var(--theme-secondary, #1e293b)' }}>
                    <h1 css={tw`text-3xl sm:text-4xl font-bold mb-6 leading-tight`}
                        style={{ color: 'var(--theme-text-primary, #f1f5f9)' }}>
                        {document.title}
                    </h1>

                    {document.updatedAt && (
                        <p css={tw`text-sm mb-8 pb-6 border-b`}
                           style={{
                    ...
                    <div css={tw`space-y-3 leading-relaxed max-w-4xl`}
                        ...
                        {lines.map((line, i) => {
                            ...
                            if (isHeading) {
                                return (
                                    <h2 key={i} css={tw`text-lg font-semibold mt-6 mb-2`}
                                        style={{ color: 'var(--theme-text-primary, #f1f5f9)' }}>
                                        {line}
                                    </h2>
                                );
                ...
                <div css={tw`text-center mt-8 text-xs`}
