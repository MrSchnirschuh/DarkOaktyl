      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Deploy your favorite game servers with one click
          </p>
        </div>
...
function TemplateCard({ template, onDeploy, formatMemory, formatDisk, featured }: TemplateCardProps) {
  return (
    <Card className={`group overflow-hidden transition-all hover:shadow-lg ${featured ? 'border-yellow-500/30' : ''}`}>
      <CardContent className="p-0">
        {/* Image or Icon Header */}
        <div className={`h-24 flex items-center justify-center relative ${TYPE_COLORS[template.type] || TYPE_COLORS.other}`}>
          {featured && (
