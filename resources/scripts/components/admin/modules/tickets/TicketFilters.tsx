    ...
    <div className="space-y-3 mb-4">
      ...
      {hasActiveFilters && (
        <div className="flex flex-wrap gap-2">
          {filters.status && (
            <Badge variant="secondary" className="cursor-pointer" onClick={() => handleFilterChange('status', null)}>
              Status: {STATUS_OPTIONS.find(s => s.value === filters.status)?.label}
              <X className="h-3 w-3 ml-1" />
            </Badge>
          )}
          {filters.priority && (
            <Badge variant="secondary" className="cursor-pointer" onClick={() => handleFilterChange('priority', null)}>
              Priorität: {PRIORITY_OPTIONS.find(p => p.value === filters.priority)?.label}
              <X className="h-3 w-3 ml-1" />
            </Badge>
          )}
          {filters.category && (
            <Badge variant="secondary" className="cursor-pointer" onClick={() => handleFilterChange('category', null)}>
