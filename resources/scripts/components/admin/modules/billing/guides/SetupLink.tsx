...
export default ({ setOpen }: { setOpen: Dispatch<SetStateAction<BillingSetupDialog>> }) => {
    ...
    return (
        <Dialog open onClose={() => setOpen('none')} title={'How to configure Link'}>
            ...
            <p className={'text-lg font-semibold mt-4 text-theme-primary'}>2. Enable Link module in Stripe</p>
            ...
            <p className={'text-lg font-semibold mt-4 text-theme-primary'}>3. Check DarkOaktyl for Link method</p>
            Finally, go back to DarkOaktyl and visit the billing store. Select a plan, and head to the checkout. You
            should now be able to checkout with Link via DarkOaktyl.
