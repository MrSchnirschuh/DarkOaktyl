...
class DatabaseSettingsCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->variables['DB_USERNAME'] = $this->option('username') ?? $this->ask(
            'Database Username',
            config('database.connections.mysql.username', 'DarkOak')
        );
        ...
        try {
            ...
            if ($this->confirm('Go back and try again?')) {
                $this->database->disconnect('_DarkOaktyl_command_test');
    ...
    {
        config()->set('database.connections._DarkOaktyl_command_test', [
            'driver' => 'mysql',
