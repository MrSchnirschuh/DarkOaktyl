...
namespace DarkOak\Console\Commands\Environment;
...
class AppSettingsCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->variables['APP_SERVICE_AUTHOR'] = $this->option('author') ?? $this->ask(
            'Egg Author Email',
            config('DarkOak.service.author', 'unknown@unknown.com')
        );
        ...
        $this->output->comment('Please reference https://DarkOaktyl.io/panel/1.0/additional_configuration.html#telemetry for more detailed information regarding telemetry data and collection.');
        $this->variables['DarkOaktyl_TELEMETRY_ENABLED'] = $this->option('telemetry') ?? $this->confirm(
            'Enable sending anonymous telemetry data?',
            config('DarkOak.telemetry.enabled', true)
        ) ? 'true' : 'false';
